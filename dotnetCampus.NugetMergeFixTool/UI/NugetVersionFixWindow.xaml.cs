    ...
    public partial class NugetVersionFixWindow : Window
    {
        public NugetVersionFixWindow(IEnumerable<VersionUnusualNugetInfoExGroup> mismatchVersionNugetInfoExs)
        {
            ...
            foreach (var mismatchVersionNugetInfoEx in _mismatchVersionNugetInfoExs)
            {
                ...
                var repeatNugetVersions = mismatchVersionNugetInfoEx.VersionUnusualNugetInfoExs.Select(x => x.Version).Distinct();
                var nugetVersionSelectorUserControl = new NugetVersionSelectorUserControl(nugetName, repeatNugetVersions);
        ...
        private void ButtonFix_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            foreach (var child in PanelNugetVersionSelectors.Children)
            {
                ...
                var selectedVersionNugetInfoExs = versionUnusualNugetInfoExGroup.VersionUnusualNugetInfoExs.Where(x => x.Version == selectedVersion);
                var targetFrameworks = selectedVersionNugetInfoExs.Where(x=>x.TargetFramework!=null).Select(x => x.TargetFramework).Distinct().ToList();
                targetFrameworks.Sort();
                targetFrameworks.Reverse();
                var nugetDllInfos = selectedVersionNugetInfoExs.Where(x => x.NugetDllInfo != null).Select(x=> x.NugetDllInfo).Distinct();
                ...
                {
                    _nugetFixStrategyList.Add(new NugetFixStrategy(nugetName, selectedVersion, targetFrameworks.First()));
                }
            }
